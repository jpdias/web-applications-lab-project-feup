...
function enableUserAccount($idreader)
{
    global $conn;
	...
	try
	{
		...
		set currentstatus=:currentstatus
		where idReader=' . $idreader . '');
		$stmt->bindParam(':currentstatus', $enabled, PDO::PARAM_STR);
		$stmt->execute();
	}
	catch (Exception $e)
	{
...
function getAllUsersByReaderId($readerid, $onlyavailableitems)
{
    global $conn;
    
	try
	{
		$whereStmt = $whereStmt . ' where idreader=' . $readerid;
		
		
		$stmt = $conn->prepare('
		select idReader, username, password, address, birthdate, email, firstname, lastname, currentstatus 
		from reader' . $whereStmt .
		'order by idReader asc');
		$stmt->execute();
		$stmt->setFetchMode(PDO::FETCH_ASSOC);
		$result = $stmt->fetchAll();
...
function getAllUsersByName($name, $onlyavailableitems)
{
    global $conn;
    
	try
	{
		$whereStmt = $whereStmt . ' where firstname=\'' . $name . '\' or lastname=\'' . $name . '\'';
...
function getAllUsersByEmail($email, $onlyavailableitems)
{
    global $conn;
    
	try
	{
		$whereStmt = $whereStmt . ' where email=\'' . $email . '\' ';
